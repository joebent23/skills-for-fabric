...
| Notebook deploys successfully but appears blank in Fabric | FabricGitSource `.py` format has incorrect whitespace or compact JSON | The `.py` format is whitespace-sensitive. Ensure blank lines after markers, multi-line JSON in `# META` blocks, no extra comments before first `# METADATA`. See [local-deployment.md § FabricGitSource Notebook Format](resources/local-deployment.md#fabricgitsource-notebook-format-critical). Or switch to `.ipynb` format |
